...
outFile = outFileDir + (outFileDir.split("/process_data/")[1]).split("/")[0] + "_PerBinWithChromSnv.tsv"
output_sum_csv = outFileDir + (outFileDir.split("/process_data/")[1]).split("/")[0] + "_PerBinWithChromSnv_sum.csv"
...
def count_fragments_per_bin(file_path, window=5000000):
...
# 创建新的sum文件，并将第一行和sum值行转置写入CSV文件
